...
func (s *PartsSupplyGateway) GetByID(ctx context.Context, id string) (*entities.PartsSupply, error) {
	logger := logs.Logger()
	...
	if err != nil {
		logger.Error().Err(err).Str("ID", id).Msg("failed to find parts supply by id")
		return nil, err
	}
